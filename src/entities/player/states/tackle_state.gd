...
func enter() -> void:
	...
	if dir.length_squared() == 0.0:
		dir = movement.last_facing_direction
...
func process_physics(delta: float) -> void:
	if _is_lunging:
		...
		if _lunge_timer <= 0.0:
			_end_lunge()
			return
		player.velocity = _lunge_direction * _lunge_speed
...
func _on_tackle_hit(hurtbox: Area2D) -> void:
	...
	if victim and "ball_control_component" in victim:
		...
		if victim_bc and victim_bc.has_ball:
			...
			if ball:
				var to_attacker: Vector2 = (player.global_position - ball.global_position).normalized()
	...
	if hurtbox.has_method(&"notify_tackled"):
		hurtbox.notify_tackled({"attacker": player, "direction": _lunge_direction})
